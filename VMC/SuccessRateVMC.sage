...
def ComGamma0WithPool(Tau, c, m, A, W, Tp):
    s = 0
    for k in range(0,Tp):
        s += binomial(Tp, k)*Tau**k*(1-Tau)**(Tp-k)*ComGamma0(ComAlpha0WithPool(Tp,k,c,m,W),ComBeta(c,m),binomial(Tp-k,W)/binomial(Tp,W), A)
    return s
...
def ComMC(Tau, Gamma0, Gamma1, A, W): 
    P = ComP(Tau, W)
    A0 = 1-2* (1-Gamma1)**(1/A)
    B0 = 1-(2/P)*Gamma0**(1/A)+((2-2*P)/P)*(1-Gamma1)**(1/A) 
...
def ComAlpha0WithPool(Tp, e, c, m, W): 
    s = 0
    for k in range(c+1, m+1):
        s += binomial(Tp - e - W, m - k)*binomial(e, k)/binomial(Tp - W, m)
    return s
